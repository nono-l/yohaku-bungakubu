...
async function createSql(): Promise<Sql> {
  ...
  if (!pgliteOk) {
    return toSql(async () => {
      throw new Error("Database unavailable in this production preview");
    });
  }
 ...
 * both backends (and again on HMR) — define tables there, never inline in server functions.
 */
 ...
 */
const pgliteOk = dbSource === "pglite" && process.env.NODE_ENV !== "production";

export function ensureDbReady(): Promise<void> {
  if (!pgliteOk) return Promise.resolve();
  return getSql()
    .then(() => undefined)
    .catch((err) => {
...
if (typeof window === "undefined" && pgliteOk) {
